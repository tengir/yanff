...
func l4ACLTCP(pkt *packet.Packet, L4 *l4Rules) bool {
	tcp := (*packet.TCPHdr)(pkt.L4)
	srcPort := packet.SwapBytesUint16(tcp.SrcPort)
	if srcPort < L4.SrcPortMin || srcPort > L4.SrcPortMax {
		return false
	}
	dstPort := packet.SwapBytesUint16(tcp.DstPort)
...
func l4ACLUDP(pkt *packet.Packet, L4 *l4Rules) bool {
	udp := (*packet.UDPHdr)(pkt.L4)
	srcPort := packet.SwapBytesUint16(udp.SrcPort)
	if srcPort < L4.SrcPortMin || srcPort > L4.SrcPortMax {
		return false
	}
	dstPort := packet.SwapBytesUint16(udp.DstPort)
	if dstPort < L4.DstPortMin || dstPort > L4.DstPortMax {
		return false
	}
	return true
...
func l3ACL(pkt *packet.Packet, rules *L3Rules) uint {
	...
	if ipv4 != nil {
		for _, rule := range rules.ip4 {
			...
			tcp, udp, _ := pkt.ParseAllKnownL4ForIPv4()
			if tcp != nil && rule.L4.valid && !l4ACLTCP(pkt, &rule.L4) {
				continue
			}
			if udp != nil && rule.L4.valid && !l4ACLUDP(pkt, &rule.L4) {
				continue
			}
			return rule.OutputNumber
		}
	...
	IPv6:
		for _, rule := range rules.ip6 {
			...
			tcp, udp, _ := pkt.ParseAllKnownL4ForIPv6()
			if tcp != nil && rule.L4.valid && !l4ACLTCP(pkt, &rule.L4) {
				continue
			}
			if udp != nil && rule.L4.valid && !l4ACLUDP(pkt, &rule.L4) {
